...
- `docs/MIGRATION-OPTIONS.md` — five candidate routes for porting/migrating off VB6 (incremental .NET via Mobilize/GAPVelocity, greenfield C#/.NET 10, Python/PySide6, Java/JavaFX, TypeScript/Tauri 2 or web), each with pros/cons, phased roadmap, tooling stack, and effort estimate. Includes a side-by-side comparison table and an opinionated recommendation (greenfield C#/.NET 10 + Avalonia + QuestPDF + SQLite). Per-route detail docs (e.g., `MIGRATION-DOTNET.md`, `MIGRATION-PYTHON.md`) are anticipated follow-ups; this overview is the route-selection doc.
