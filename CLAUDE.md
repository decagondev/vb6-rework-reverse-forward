...
- `docs/MIGRATION-OPTIONS.md` — five candidate routes for porting/migrating off VB6 (incremental .NET via Mobilize/GAPVelocity, greenfield C#/.NET 10, Python/PySide6, Java/JavaFX, TypeScript/Tauri 2 or web), each with pros/cons, phased roadmap, tooling stack, and effort estimate. Includes a side-by-side comparison table and an opinionated recommendation (greenfield C#/.NET 10 + Avalonia + QuestPDF + SQLite). Per-route detail docs are anticipated follow-ups; this overview is the route-selection doc.
- `docs/MIGRATION-DOTNET.md` — implementation playbook for the recommended route. Defines target architecture (SOLID + layered: Domain/Application/Infrastructure/Presentation/Desktop), tech stack (.NET 10 + Avalonia 11 + EF Core + SQLite + QuestPDF + Argon2id + Serilog), coding standards, test strategy (unit + integration + smoke as the regression gate), Git workflow (slice branches, conventional commits, squash-merge to `main`), and the **slice loop** — a strict protocol where the implementer creates a slice branch, writes code+tests+docs, runs all test gates, then hands off to the user for review/stage/commit/merge. Implementer never commits to `main` directly. Includes a 14-arc roadmap (Foundation → Data → Auth → Domain → Reports → Presentation → Login/Dashboard → Booking → Find Customer/Reports → Admin Tools → Cross-cutting → Migration Tooling → Polish → Release) and templates for arc files, DEV-LOG entries, and slice plans.
- `docs/DEV-LOG.md` — append-only chronological journal, one entry per slice. Required artifact during the rewrite. Created empty; populated as work progresses.
- `docs/arcs/` — per-arc planning files (`ARC-NN-NAME.md`). Each arc is written before its slices begin, listing stories, slices, acceptance criteria, and which `BUGS-MITIGATIONS.md` items it resolves. Created empty; arc files added as work begins.
